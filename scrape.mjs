// scrape.mjs – fetches showtimes from Vue Nottingham and saves vue-nottingham.json
...
(async () => {
  ...
  try {
    ...
    // scrape visible text — we’ll search it for times and titles
    ...
    for (const line of lines) {
      ...
      const screen = screenMatch
        ? /^\d+$/.test(screenMatch[1])
          ? Number(screenMatch[1])
          : screenMatch[1]
        : null;
  ...
  // figure out next starting & finishing shows
  ...
  const next_finishing = current
    ? {
        ...
        start: fmt(current._startM),
        end: fmt(current._endM),
      }
    : null;
